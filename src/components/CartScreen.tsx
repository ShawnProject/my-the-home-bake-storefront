...
const CartScreen: React.FC<CartScreenProps> = ({
  ...
  const [viewDate, setViewDate] = useState(new Date(deliveryDate));
  ...
  const isSelected = (d: Date) => d.getDate() === deliveryDate.getDate() && d.getMonth() === deliveryDate.getMonth() && d.getFullYear() === deliveryDate.getFullYear();
        ...
        <div className="px-4 mt-6">
          ...
          <div className="bg-white dark:bg-[#2d221a] rounded-xl p-4 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <span onClick={handlePrevMonth} className="material-symbols-outlined cursor-pointer hover:text-primary transition-colors">chevron_left</span>
              <p className="text-base font-bold">{monthNames[viewDate.getMonth()]} {viewDate.getFullYear()}</p>
              <span onClick={handleNextMonth} className="material-symbols-outlined cursor-pointer hover:text-primary transition-colors">chevron_right</span>
            </div>
                ...
                <div key={i} className="flex justify-center">
                  {date ? (
                    <button
                      ...
                      className={`h-9 w-9 rounded-full flex items-center justify-center text-sm transition-all ${isSelected(date) ? 'bg-primary text-white font-bold' : 'font-medium hover:bg-orange-50 dark:hover:bg-white/5'
                        }`}
                    >
                      {date.getDate()}
                    </button>
